...
public final class MIDIManager: @unchecked Sendable {
    ...
    nonisolated(unsafe) public var onSetupChanged: (@Sendable () -> Void)?

    nonisolated(unsafe) private var inputCallback: (@Sendable (MIDIMessage) -> Void)?
    ...
    public func createInputPort(name: String, callbackQueue: DispatchQueue = .main, callback: @escaping @Sendable (MIDIMessage) -> Void) throws {
    ...
    private func handleEventList(_ eventListPtr: UnsafePointer<MIDIEventList>) {
        ...
        var current = UnsafeMutablePointer(mutating: firstPacketPtr)
    ...
    private func parseAndDispatchPacket(_ packet: MIDIEventPacket) {
        ...
        if messageType == 0x02 { // MIDI 1.0 channel voice
            ...
            let _ = UInt8((word >> 20) & 0x0F)  // sysExStatus: 0=complete, 1=start, 2=continue, 3=end
